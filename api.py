...
book = {
    ...
    "DE": {"address1": "Friedrichstr 123", "city": "Berlin", "postalCode": "10117", "zoneCode": "BE", "countryCode": "DE", "phone": "3012345678"},
...
async def autoshopify(url, card, session):
    ...
    try:
        ...
        # Get payment session token using the CORRECT endpoint
        headers = {
            'accept': 'application/json',
            'accept-language': 'en-IN,en-GB;q=0.9,en-US;q=0.8,en;q=0.7',
            'content-type': 'application/json',
        ...
        # Use the WORKING payment sessions endpoint
        ...
        # Extract shipping handle and amount
        ...
        # Get shipping handle
        handle_match = re.search(r'"handle"\s*:\s*"([^"]+)"', request.text)
        handle = handle_match.group(1) if handle_match else None

        # Get delivery amount
        amount_match = re.search(r'"amount"\s*:\s*{\s*"value"\s*:\s*{\s*"amount"\s*:\s*"([\d.]+)"', request.text)
        ...
        # Second request to stabilize
        request = await session.post(f'{url}/checkouts/unstable/graphql', params=params, headers=headers, json=json_data)
        
        # Get running total
        total_match = re.search(r'"runningTotal"\s*:\s*{[^}]*"value"\s*:\s*{[^}]*"amount"\s*:\s*"([\d.]+)"', request.text)
        total = total_match.group(1) if total_match else str(price)
        ...
        json_data = {
            'query': 'mutation SubmitForCompletion($input:NegotiationInput!,$attemptToken:String!,$metafields:[MetafieldInput!],$postPurchaseInquiryResult:PostPurchaseInquiryResultCode,$analytics:AnalyticsInput){submitForCompletion(input:$input attemptToken:$attemptToken metafields:$metafields postPurchaseInquiryResult:$postPurchaseInquiryResult analytics:$analytics){...on SubmitSuccess{receipt{...ReceiptDetails __typename}__typename}...on SubmitAlreadyAccepted{receipt{...ReceiptDetails __typename}__typename}...on SubmitFailed{reason __typename}...on SubmitRejected{buyerProposal{...BuyerProposalDetails __typename}sellerProposal{...ProposalDetails __typename}errors{...on NegotiationError{code localizedMessage nonLocalizedMessage localizedMessageHtml __typename}__typename}__typename}...on Throttled{pollAfter pollUrl queueToken __typename}...on CheckpointDenied{redirectUrl __typename}...on SubmittedForCompletion{receipt{...ReceiptDetails __typename}__typename}__typename}}fragment ReceiptDetails on Receipt{...on ProcessedReceipt{id token orderIdentity{id __typename}__typename}...on ProcessingReceipt{id pollDelay __typename}...on WaitingReceipt{id pollDelay __typename}...on ActionRequiredReceipt{id action{...on CompletePaymentChallenge{offsiteRedirect url __typename}...on CompletePaymentChallengeV2{challengeType challengeData __typename}__typename}__typename}...on FailedReceipt{id processingError{...on PaymentFailed{code messageUntranslated __typename}__typename}__typename}__typename}fragment BuyerProposalDetails on Proposal{__typename}fragment ProposalDetails on Proposal{__typename}',
            'variables': {
                'input': {
                    'sessionInput': {
                        'sessionToken': x_checkout_one_session_token,
        ...
        request = await session.post(f'{url}/checkouts/unstable/graphql', params=params, headers=headers, json=json_data)
        res_json = request.text
        
        # Parse response
        result = capture(res_json, '"code":"', '"')
        
        if 'ProcessedReceipt' in res_json and 'processingError' not in res_json.lower():
            output.update({
        ...
        elif "insufficient_funds" in str(res_json).lower() or "INSUFFICIENT_FUNDS" in str(res_json):
            output.update({
        ...
        elif "INVALID_ACCOUNT" in str(res_json):
            output.update({
        ...
        elif "CARD_NOT_SUPPORTED" in str(res_json):
            output.update({
        ...
        elif "PROCESSING_ERROR" in str(res_json):
            output.update({
        ...
        elif "CALL_ISSUER" in str(res_json):
            output.update({
                "Response": "CALL_ISSUER",
                "Status": True,
                "Gateway": gateway,
                "Price": total,
                "cc": card
            })
        elif "PICK_UP_CARD" in str(res_json):
            output.update({
        ...
        elif "TRY_AGAIN_LATER" in str(res_json):
            output.update({
